...
| 4 | Billing (payments/checkout, Stripe webhook linked to sessions) | In review | Branch `nestjs/phase-4-billing` → `feature/nestjs-migration`. `POST /v1/payments/checkout` replaces Rails' oddly-nested `/payments/billing/checkout`. Session linkage (checkout → `metadata.session_id` → webhook marks `sessions.paid = true`) is genuinely new functionality — Rails' checkout never took a session param and its webhook handler was a total no-op, so there was no existing behavior to preserve. New `sessions.paid` column via a real migration (`AddPaidToSessions`) — the first schema change this app has made beyond recreating Rails' existing tables. Needed `rawBody: true` for Stripe signature verification; caught two test-authoring bugs along the way (a non-constructable arrow-function mock, and `createNestApplication(undefined, opts)` silently discarding `opts`). Verified locally: build, lint, unit + 27 e2e tests (checkout mocks the `stripe` package since it's a real API call; webhook verification is tested for real with a hand-computed signature), plus a manual end-to-end smoke test hitting the real webhook endpoint and confirming `paid` flips in the database. |
