...
| 3 | Recorder integration (JWT guard, heartbeat/webhook, clip job wiring) | In review | Branch `nestjs/phase-3-recorder-integration` → `feature/nestjs-migration`. Closes the Phase 2 gap: `stop` now really enqueues a BullMQ `clip-request` job, processed by calling the recorder over HTTP. `RecorderAuthGuard` throws 401 (not Nest's guard-default 403) to match Rails' `head :unauthorized`. Outbound/inbound recorder wire format stays snake_case (external contract) even though this app's own API is camelCase. Verified locally: build, lint, unit tests (guard + recorder-client with a mocked `fetch`), e2e tests (20, including asserting the job actually lands in the queue and the full webhook flow against a real Postgres+Redis), and a real end-to-end smoke test — booted the app, hit `stop`, and confirmed a real BullMQ worker POSTed the correct signed-JWT + snake_case payload to a throwaway local HTTP server standing in for the recorder. |
